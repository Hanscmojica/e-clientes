...
router.get('/usuarios', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const usuariosFormateados = usuarios.map(usuario => {
      ...
      // Mapeo especial para HANS
      let clienteId = usuario.nId01Usuario;
      if (usuario.sUsuario.toUpperCase() === 'HANS') {
        clienteId = 5951;
      }
      ...
      return {
        ...
        lastLogin: usuario.dUltimoAcceso?.toISOString() || null,
...
router.post('/usuarios', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (perfil) {
      ...
      if (perfilObj) {
        await prisma.BP_02_PERFIL_USUARIO.create({
          data: {
            nIdUsuario: nuevoUsuario.nId01Usuario,
            nIdPerfil: perfilObj.nId03Perfil
          }
        });
      }
    }
    ...
    const clienteId = username.toUpperCase() === 'HANS' ? 5951 : nuevoUsuario.nId01Usuario;
...
router.put('/usuarios/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    let usuario;
    if (userId === 5951) {
      // Caso especial para HANS
      usuario = await prisma.BP_01_USUARIO.findFirst({
        where: { sUsuario: 'HANS' }
      });
    } else {
      usuario = await prisma.BP_01_USUARIO.findUnique({
    ...
    const clienteId = usuarioActualizado.sUsuario.toUpperCase() === 'HANS' ? 5951 : usuarioActualizado.nId01Usuario;
...
router.delete('/usuarios/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    await prisma.BP_02_PERFIL_USUARIO.deleteMany({
      where: { nIdUsuario: usuario.nId01Usuario }
    });
